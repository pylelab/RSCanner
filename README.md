...
RSCanner uses a centered sliding window (default=51nt, 25nt flanking at each side) to calculate BPC and median Shannon Entropy. If an even number window size is inputted, RSCanner will use the next odd number as the window size (i.e., if input=50, the actual window size=51). In this way, a centered sliding window is always used in all calculations. When dealing with the end, RSCanner will gradually truncate the window size. (i.e., if window size=51, window #26=[1nt,51nt]; window#25=[1nt,50nt]; window#24 =[1nt,49nt].. The 3’end windows are computed in the same manner.)
